  ...
  const blendProgress = useMemo(() => {
    ...
    BLEND_ORDER.forEach((blend) => {
      const blendEvents = events.filter(
        (event) =>
          event.lesson === "language-arts:consonant-blends:double-alphabet" && event.activity === `blend-${blend}`,
      );
    ...
    <div className="relative min-h-screen bg-[radial-gradient(circle_at_top,#eef6ff,#f9fcff_55%,#f3f8ff)]">
      ...
      <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-10">
        <header className="space-y-3 text-center">
          ...
          <h1 className="font-display text-4xl font-semibold text-stone-900">Consonant Blend Picture Cards</h1>
        ...
        <section className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {BLEND_ORDER.map((blend) => {
            ...
            return (
              <Link
                key={blend}
                href={ready ? `/lessons/language-arts/consonant-blends/double-alphabet/${blend}` : "#"}
                aria-disabled={!ready}
