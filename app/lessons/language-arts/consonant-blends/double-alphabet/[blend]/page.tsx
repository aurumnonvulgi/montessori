...
const buildFallbackFiles = () =>
  BLEND_ORDER.flatMap((blend) =>
    BLEND_WORDS[blend].map((word, index) => `${blend}-${word}-${index + 1}-${word}____consonant_blends.png`),
  );
...
export default function ConsonantBlendDoubleAlphabetLesson() {
  ...
  useEffect(() => {
    trackLessonEvent({
      lesson: "language-arts:consonant-blends:double-alphabet",
  ...
  return (
    <div className="relative min-h-screen bg-[radial-gradient(circle_at_top,#eef6ff,#f9fcff_55%,#f3f8ff)]">
      ...
      <CompletionOverlay
        ...
        primaryAction={{ href: "/lessons/language-arts/consonant-blends/double-alphabet", label: "All Blends" }}
