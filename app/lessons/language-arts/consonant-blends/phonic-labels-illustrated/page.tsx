  ...
  const blendProgress = useMemo(() => {
    ...
    BLEND_ORDER.forEach((blend) => {
      const blendEvents = events.filter(
        (event) =>
          event.lesson === "language-arts:consonant-blends:phonic-labels-illustrated" && event.activity === `blend-${blend}`,
      );
        ...
        <section className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {BLEND_ORDER.map((blend) => {
            ...
            return (
              <Link
                key={blend}
                href={ready ? `/lessons/language-arts/consonant-blends/phonic-labels-illustrated/${blend}` : "#"}
                aria-disabled={!ready}
