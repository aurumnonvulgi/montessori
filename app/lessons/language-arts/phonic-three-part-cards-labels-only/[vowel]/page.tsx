...
export default function PhonicThreePartCardsLabelsOnlyLesson() {
  ...
  const completionSteps = useMemo(
    () => getPhonicsCompletionSteps("phonic-three-part-cards-labels-only", vowel),
    [vowel]
  );
  ...
  useEffect(() => {
    ...
    const handleUp = (event: PointerEvent) => {
      ...
      if (candidates.length && candidates[0].dist <= DROP_THRESHOLD) {
        ...
        const targetIndex = activeLabelSlots.findIndex((slot) => slot.id === target.id);
  ...
  }, [convertPointerToBoard, dragging, labelAssignments, labelCards, stageCount, stageIndex, stagePairs, vowel]);
  ...
  }, [handleSpeak, labelMatches, stageCount, stageIndex, vowel]);
      ...
      <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-10">
        <header className="space-y-3 text-center">
          <p className="text-xs uppercase tracking-[0.35em] text-stone-400">
            Phonic Three-Part Cards Labels · Vowel {vowel}
          </p>
          <h1 className="font-display text-4xl font-semibold text-stone-900">
            Match Labels to Picture + Three-Part Card
          </h1>
          <p className="text-sm text-stone-600">Drag each label to the matching picture and full card set.</p>
        </header>
              ...
              return (
                ...
                >
                  <div className="relative h-full w-full rounded-md bg-white/90 p-0.5 sm:p-1 shadow-[0_8px_18px_-14px_rgba(0,0,0,0.8)]">
                    <img src={toLabelImage(pair.labelFile)} alt={pair.wordLabel} className="h-full w-full object-contain" />
                    <button
                      type="button"
