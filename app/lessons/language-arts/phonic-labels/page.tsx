...
const toPictureImage = (letter: string, word: string) =>
  `/assets/language_arts/moveable_alphabet/images/${letter}---${word}___moveable_phonics.png`;

export default function PhonicLabelsHub() {
  return (
    <div className="relative min-h-screen bg-[radial-gradient(circle_at_top,#f5efe6,#fdfbf8_55%,#f7efe4)]">
      <HomeLink />
      <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-10">
        <header className="space-y-3 text-center">
          <p className="text-xs uppercase tracking-[0.35em] text-stone-400">Language Arts · Phonic Labels</p>
          <h1 className="font-display text-4xl font-semibold text-stone-900">Match Label to Picture</h1>
          <p className="text-sm text-stone-600">Choose a vowel group to start matching.</p>
        </header>
        <div className="grid gap-6 md:grid-cols-2">
          {VOWELS.map((vowel) => (
            <Link
              key={vowel.letter}
              href={`/lessons/language-arts/phonic-labels/${vowel.letter}`}
